...
import {
  exportPackagePerformanceCsv,
  getPackagePerformance,
  getPackageRevenueOverview,
...
import PackagePerformanceStats from "./package-performance-stats";
import PackagePerformanceTable from "./package-performance-table";

interface PackagePerformanceClientProps {
...
const getErrorMessage = (error: unknown) => {
  return error instanceof Error
    ? error.message
    : "Unable to load package revenue performance.";
};
...
export default function PackagePerformanceClient({
  preset,
  from,
  to,
  page,
  ...
  sortBy,
  sortOrder,
}: PackagePerformanceClientProps) {
  const router = useRouter();
  const pathname = usePathname();

  const [overview, setOverview] = useState<PackageOverviewResponse | null>(
    null,
  );

  const [performance, setPerformance] =
    useState<PackagePerformanceResponse | null>(null);
  ...
  const listQuery = useMemo(
    ...
    [limit, packageType, page, periodQuery, search, sortBy, sortOrder, status],
  );
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [overviewResponse, performanceResponse] = await Promise.all([
        getPackageRevenueOverview(periodQuery),

        getPackagePerformance(listQuery),
      ]);
  ...
  const handleExport = async () => {
    const toastId = toast.loading("Preparing package performance CSV...");

    try {
      setIsExporting(true);

      const csv = await exportPackagePerformanceCsv({
        ...periodQuery,
      ...
      downloadCsv(csv, "package-performance.csv");

      toast.success("Package performance CSV downloaded.", {
        id: toastId,
      });
    } catch (error) {
    ...
    return (
      <div className="mx-auto flex min-h-[460px] max-w-[760px] flex-col items-center justify-center rounded-[2.5rem] bg-white p-8 text-center shadow-xl shadow-black/5">
        ...
        <h2 className="mt-5 text-2xl font-bold text-[#202420]">
          Package performance unavailable
        </h2>
  ...
  return (
    <section className="space-y-7">
      ...
      <PackagePerformanceStats overview={overview} />

      <PackagePerformanceTable
        key={`${search}-${packageType || "all"}-${status || "all"}-${sortBy}-${sortOrder}-${limit}`}
