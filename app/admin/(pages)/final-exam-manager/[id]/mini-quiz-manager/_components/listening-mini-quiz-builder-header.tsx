...
interface ListeningMiniQuizBuilderHeaderProps {
  courseTitle: string;
  examTitle: string;
...
const ListeningMiniQuizBuilderHeader = ({
  ...
  onBack,
  onPublish,
}: ListeningMiniQuizBuilderHeaderProps) => {
  const breadcrumbs = [
    "Course Manager",
    "Exam",
    ...
    <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
      <div>
        ...
        <h1 className="text-2xl font-bold text-[#006B3F] md:text-3xl">
          Listening Mini Quiz Builder: {examTitle}
        </h1>

        <p className="mt-1 text-sm text-[#4F5B55]">
          Manage listening comprehension audio, prompts, and answer options.
        </p>
      </div>
